...
NOT_TOKEN_CREATOR_ERROR = f"typed error: {web3.solidityKeccak(['string'], ['NotTokenCreator()']).hex()[:10]}"
...
def test_story_contract_not_creator(story_contract):
    for i in range(1, 4):
        with reverts(NOT_TOKEN_CREATOR_ERROR):
            story_contract.addCreatorStory(i, "XCOPY", "I AM XCOPY", {"from": accounts[i]})
...
def test_story_contract_nonexistent_token(story_contract):
    for i in range(1, 4):
        with reverts(TOKEN_DOES_NOT_EXIST_ERROR):
            story_contract.addStory(i + 4, "NOT XCOPY", "I AM NOTXCOPY", {"from": accounts[i]})
